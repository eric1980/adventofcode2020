...
def visible_occupied(seats, row, col):
    ...
    return sum([get_visible_seat(seats, row, col, d) == '#' for d in directions])


def get_visible_seat(seats, row, col, direction):
    ...
    while seats[check_row][check_col] == '.':
        if not 0 < check_row < max_rows - 1 or not 0 < check_col < max_cols - 1:
            break
        check_row += delta_row
        check_col += delta_col
